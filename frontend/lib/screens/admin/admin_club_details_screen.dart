...
import 'package:campus_club_manager/screens/admin/admin_dashboard_screen.dart';
...
class _AdminClubDetailsScreenState extends State<AdminClubDetailsScreen> {
  ...
  final TextEditingController _addMemberNameController =
      TextEditingController();
  ...
  Future<void> _addMember() async {
    ...
    try {
      ...
      if (response.statusCode == 201) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text(data['message'] ?? 'Failed to add member')),
        );
      }
    } catch (e) {
      Navigator.pop(context);
  ...
  Future<void> _deleteClub() async {
    try {
      ...
      final response = await http.delete(
        Uri.parse('$_apiBaseUrl/api/clubs/${widget.clubId}'),
        headers: headers,
      );
      if (response.statusCode == 200 || response.statusCode == 204) {
        ...
        Navigator.push(context, MaterialPageRoute(builder: (context) => AdminDashboardScreen(token: widget.token,)));
      } else {
        final data = jsonDecode(response.body);
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text(data['message'] ?? 'Failed to delete club')),
        );
      }
    } catch (e) {
      ScaffoldMessenger.of(
          ...
          ? Center(
              child: Column(
                ...
                children: [
                  ...
                  ElevatedButton(
                    onPressed: _loadClubDetailsAndMembers,
